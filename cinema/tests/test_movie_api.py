...
class PrivateMoviesApiTests(APITestCase):
    ...
    def test_list_movies_shows_image_url(self):
        Movie.objects.create(title="Liar", description="d", duration=90)
        res = self.client.get(MOVIES_URL)
...
class PrivateMoviesPermissionsTests(APITestCase):
    ...
    def test_update_movie_forbidden_for_non_admin(self):
        m = Movie.objects.create(title="Old", description="d", duration=90)
        url = detail_url(m.id)
        res = self.client.patch(url, {"title": "Hack"})
        self.assertEqual(res.status_code, status.HTTP_403_FORBIDDEN)

    def test_delete_movie_forbidden_for_non_admin(self):
        m = Movie.objects.create(title="Keep", description="d", duration=90)
        url = detail_url(m.id)
...
class AdminMoviesApiTests(APITestCase):
    ...
    def test_admin_can_create_movie_but_no_image_field(self):
        ...
        # Для ImageField відсутність файлу перевіряємо через "falsy" або .name is None
    ...
    def test_upload_image_endpoint_exists(self):
        ...
        url = upload_url(m.id)
        # без файлу має бути 400 або 415 (залежно від парсера/headers)
    ...
    def test_delete_movie_as_admin(self):
        m = Movie.objects.create(title="ToDelete", description="d", duration=90)
        url = detail_url(m.id)
        res = self.client.delete(url)
...
class AdminMoviesValidationTests(APITestCase):
    ...
    def test_create_movie_invalid_payload_returns_400(self):
        payload = {
            "title": "",     # невалідно: порожній
            "description": "d",
            "duration": -5,  # невалідно: від’ємна тривалість
        }
        res = self.client.post(MOVIES_URL, payload)
        self.assertEqual(res.status_code, status.HTTP_400_BAD_REQUEST)
        # очікуємо, що серіалізатор підсвітить помилки
    ...
    def test_update_movie_invalid_payload_returns_400(self):
        ...
        url = detail_url(m.id)
        res = self.client.patch(url, {"duration": -10})
        self.assertEqual(res.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("duration", res.data)
